...
export const businessRegistrationSchema = z.object({
  ...
  state: z.string().refine(val => val !== "select-state", "State is required"),
  zip: z.string().min(1, "ZIP code is required").regex(/^\d{5}(-\d{4})?$/, "ZIP code must be in XXXXX or XXXXX-XXXX format"),
  industry: z.string().refine(val => val !== "select-industry", "Industry is required"),
  size: z.string().refine(val => val !== "select-range", "Company size is required"),
